...
export default function ClientRegisterScreen({ navigation }) {
    ...
    const { registerClient } = useContext(AppContext);

    const handleRegister = async () => {
        if (name && password) {
            await registerClient(name, password, gender);
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Müşteri Kaydı</Text>
            ...
            <TouchableOpacity style={styles.button} onPress={handleRegister}>
                <Text style={styles.btnText}>Kayıt Ol ve Pazaryerine Git</Text>
            </TouchableOpacity>
        </View>
    );
}
...
const styles = StyleSheet.create({
    ...
    activeGender: { borderColor: '#f093fb', backgroundColor: 'rgba(240, 147, 251, 0.1)' },
    ...
    button: { backgroundColor: '#f093fb', padding: 15, borderRadius: 10, alignItems: 'center', marginTop: 10 },
